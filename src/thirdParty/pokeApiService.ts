...
export async function getPokedexByGeneration(generation: string): Promise<Pokedex> {
  return (
...
export async function getPokemonInformationById(pokemonId: number): Promise<CaughtPokemon> {
  ...
  const pokemonDescription: PokemonDescription = await getPokemonDescriptionById(pokemonId);
...
export async function getPokemonDescriptionById(pokemonId: number): Promise<PokemonDescription> {
  ...
  const description = flavor_text_entries.find(
    (textEntry) => textEntry.language.name === 'en' && textEntry.version.name === 'red',
  );
