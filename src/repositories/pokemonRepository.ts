...
export async function guardar(nombrePokemon: string): Promise<PokemonAtrapado> {
    return await prisma.pokemonAtrapado.create({ data: { nombre: nombrePokemon } });
}
...
export async function obtenerActual(): Promise<PokemonAtrapado> {
    ...
    return await prisma.pokemonAtrapado.findFirstOrThrow({ where: { fecha: { gte: fechaHoyISOString, lte: fechaMananaISOString } } })
}
