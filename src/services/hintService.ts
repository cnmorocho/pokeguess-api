...
export async function updateHint(hintId: number, tries: number): Promise<Hint> {
  ...
  switch (tries) {
    case 1:
      return await updateHintById(hintId, { type });
    case 2:
      return await updateHintById(hintId, { color });
    case 3:
      return await updateHintById(hintId, { category });
    case 4:
      return await updateHintById(hintId, { description });
    case 5:
      return await updateHintById(hintId, { sprite });
    default:
